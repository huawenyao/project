...
-- 注释
COMMENT ON TABLE agent_error_record IS 'Agent错误记录表 - 追踪错误和恢复历史';
COMMENT ON COLUMN agent_error_record.error_type IS 'minor(自动重试3次), critical(立即暂停)';
COMMENT ON COLUMN agent_error_record.resolution IS '最终处理方式';
